...
def get_custom_rabbitmq_recipe_message_ids():
  try:
    query = " MATCH(recipe_data_message_id: RabbitMQ_Recipe_Data_Message_id) RETURN recipe_data_message_id; "
    results,meta = db.cypher_query(query=query)
    print("Results: ",results)
...
def add_consumed_rabbitmq_recipe_message_id(message_id, created_at):
  try:
    query = " CREATE(recipe_data_message_id: RabbitMQ_Recipe_Data_Message_id{ message_id: $message_id, created_at: $created_at }) RETURN recipe_data_message_id; "
    db.cypher_query(query=query, params={"message_id": message_id, "created_at": created_at})
    print("Message id node inserted successfully")
  except NeomodelException as e:
    print("Error inserting new custom rabbitmq user data message id node: ", e)